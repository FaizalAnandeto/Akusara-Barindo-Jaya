...
async function fetchJson<T>(url: string, init?: RequestInit): Promise<T> {
  try {
    ...
    if (!res.ok) {
      ...
      console.error('[finance api] request failed', { url, status: res.status, body: text });
...
export async function fetchFinanceOverview(): Promise<FinanceOverview> {
  return fetchJson(`${API_BASE_URL}/finance/overview?ts=${Date.now()}`, { cache: 'no-store' });
}

export async function fetchResidents(): Promise<Resident[]> {
  return fetchJson(`${API_BASE_URL}/finance/residents?ts=${Date.now()}`, { cache: 'no-store' });
}

export async function fetchTransactions(): Promise<Transaction[]> {
  const data: Transaction[] = await fetchJson(`${API_BASE_URL}/finance/transactions?ts=${Date.now()}`, { cache: 'no-store' });
  return data.map(t => ({ ...t, type: (t as any).tx_type || t.type }));
}

export async function addTransaction(payload: { resident: string; block: string; amount: number; type: string; method: string; notes?: string }): Promise<{ id: string }>{
...
export async function deleteTransaction(id: string): Promise<{ status: string }> {
...
export async function fetchFinanceState(): Promise<FinanceState> {
  return fetchJson(`${API_BASE_URL}/finance/state?ts=${Date.now()}`, { cache: 'no-store' });
}

// Simple client-side export helpers
export async function exportTransactionsCSV(): Promise<void> {
  const data = await fetchTransactions();
  const headers = ['ID','Resident','Block','Amount','Type','Method','Date','Time','Status'];
  const rows = data.map(d => [d.id, d.resident, d.block, d.amount, (d.type || d.tx_type || ''), d.method, d.date, d.time, d.status]);
  ...
  a.href = url; a.download = `transactions_${Date.now()}.csv`; document.body.appendChild(a); a.click(); a.remove();
...
export async function exportTransactionsPDF(): Promise<void> {
  ...
  ${data.map(d => `<tr><td>${d.id}</td><td>${d.resident}</td><td>${d.block}</td><td>${d.amount}</td><td>${d.type || d.tx_type || ''}</td><td>${d.method}</td><td>${d.date}</td><td>${d.time}</td><td>${d.status}</td></tr>`).join('')}
  </tbody></table></body></html>`;
  // We download HTML which user can "Save as PDF" via browser dialog
  const blob = new Blob([html], { type: 'text/html;charset=utf-8' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = `transactions_${Date.now()}.html`; document.body.appendChild(a); a.click(); a.remove();
  URL.revokeObjectURL(url);
}
...
export async function exportTransactionsExcelXLS(): Promise<void> {
  ...
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = `transactions_${Date.now()}.xls`; document.body.appendChild(a); a.click(); a.remove();
  URL.revokeObjectURL(url);
}
